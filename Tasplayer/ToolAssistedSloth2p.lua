...
function write_savestate_to_file(num)
  file = io.open(savedir .. "/" .. "savestates/save_" .. num .. ".mst", "wb")
  file:write(emu.saveSavestate())
  file:close()
end

function read_savestate_from_file(num)
  state_to_load = io.open(savedir .. "/" .. "savestates/save_" .. num .. ".mst", "rb"):read("*a")
  emu.reset()
  set_to_load_reset = true
end
...
function run_tas()
  ...
  ::set_inputs::
  just_loaded = false
  emu.setInput(playernumber, orray(input, hold))
  draw_input_text()
  if running then
   save_tas_state("RUN")
  end
...
function draw_input_text()
  ...
  local startx = 0 + playernumber*200
...
function loop_savestates(num, getcount)
  ...
  while true do
    ...
    if c == "!" then
      ...
      last_save_line = ln
      last_save_char = cn
      if not file_exists(savedir .. "/" .. "savestates/save_" .. statenum .. ".mst") then
        log("File: savestates/save_" .. statenum .. ".mst' not found!")
        return false or getcount
      end
      if getcount then
...
function draw_string(s, x, y, color, bg, duration)
  ...
  if drawonscreen then
    emu.drawString(x, y -playernumber*10, s, color, bg, duration)
  end
end
...
function get_config()
  ...
  if string.sub(basedir, #basedir-7) == "/Player1" or string.sub(basedir, #basedir-7) == "/Player2" then
    savedir = string.sub(basedir,1, #basedir-7)
  else
    savedir = basedir
  end
  return true
...
function init()
  ...
  if file_exists(basedir) then
    if not file_exists(basedir .. "/" .. "savestates") then
      os.execute("mkdir " .. savedir .. "/" .. "savestates")
